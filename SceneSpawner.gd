...
var _spawnOrderCounter:int = 0:
	set(val):
		...
		elif val >= spawnOrder.size(): _spawnOrderCounter = 0 #push_error( "Index out of bounds. spawnOrder size is {0} but the value was {1}".format([spawnOrder.size(), val]) )
...
func spawn_scene():
	...
	spawned_node.emit(node, node.position)
...
func validate_spawn_order()->bool:
	...
	if anyOutOfBounds: push_error("spawnOrder has out of bounds integers. Switching to random mode."); spawnOrder = []; return false
	else: return true
...
func get_next_node()->Node:
	...
	if spawnOrder.is_empty():
		node = scenesToSpawn.pick_random().duplicate().instantiate()
	else:
		node = scenesToSpawn[_spawnOrderCounter].instantiate()
